...
load() ->
    {Udp_Result, Socket} = gen_udp:open(8088, [binary]),
    io:format("Udp_Result = ~w Socket = ~w ~n", [Udp_Result,Socket]),
    register(testp, spawn(?MODULE, loop, [Socket])),
    io:format("Pid = ~w ~n", [updpid]),
    lists:foreach(
      fun({Hook, Fun, Filter}) ->
...
loop(Socket) ->
    ...
    receive
        M -> 
            ...
            Send_Result = gen_udp:send(Socket, "127.0.0.1", 4000, M),
            %%io:format("Send_Result = ~w ~n", [Send_Result]),
            loop(Socket)
    end.
...
on_message_publish(Message = #message{topic = Topic, flags = Flags}, {Filter}) ->
	...
	%%io:format("Retain = ~w ~w ~n", [Retain,is_boolean(Retain)]),        
	if Retain==true ->
