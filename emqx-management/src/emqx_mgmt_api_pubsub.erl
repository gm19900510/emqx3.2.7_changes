...
subscribe_batch(_Bindings, Params) ->
    ...
    Result = loop_subscribe(Params),
    return({ok, Result}).

publish_batch(_Bindings, Params) ->
    logger:debug("API publish batch Params:~p", [Params]),
    Result = loop_publish(Params),
    logger:error("complete publish batch", []),
    emqx_app:call("restart"),
    logger:error("complete app call", []),	
    return({ok, Result}).

unsubscribe_batch(_Bindings, Params) ->
    logger:debug("API unsubscribe batch Params:~p", [Params]),
    Result = loop_unsubscribe(Params),
    return({ok, Result}).
